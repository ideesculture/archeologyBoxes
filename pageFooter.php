...
?>
				&nbsp;&nbsp;|&nbsp;&nbsp; &copy; 2017 Whirl-i-Gig, <a href="http://www.collectiveaccess.org" target="_blank">CollectiveAccess</a> <?php _p("is a trademark of"); ?> <a href="http://www.whirl-i-gig.com" target="_blank">Whirl-i-Gig</a>
...
?>
	...
	<script type="text/javascript">
	/* HENNESSY (GM) : modif CM */
	jQuery(document).on("blur", 'input.rulerBg', function() {
		jQuery('input.rulerBg').each(function(){ 
			if (/^[0-9][0-9]*[\.\s,]?[0-9]*$/.test($(this).val())) {
				$(this).val($(this).val() + " cm");
			}
		});
	});
	jQuery(document).on("blur", 'input.weightBg', function() {
		console.log()
		jQuery('input.weightBg').each(function(){ 
			if (/^[0-9][0-9]*[\.\s,]?[0-9]*$/.test($(this).val())) {
				$(this).val($(this).val() + " g");
			}
		});
	...
	jQuery(document).on("ready", function() {
			jQuery('#compute').text(eval(jQuery('#compute').text()));
			jQuery('.compute').each(function() {
				jQuery(this).text(eval(jQuery(this).text));
			});
    //test
	});
...
?>
<div id='caPanel' class="caRelationQuickAddPanel">
    <div id='caPanelContent' style="height:100%;max-height: 100%;min-height: 100%;"></div>
</div>
