<div style="background:white;display:block;"><?php
    ...
    $types_caisses = $this->getVar("types_caisses");
?>
	<h3 style="margin-top: 0;">Ajouter à une nouvelle caisse</h3>
<?php
?>	<form method="post" action="<?php print $addBox_url; ?>" id="addBox">
		<div style="background-color:#F7F7F7;padding:20px 20px 5px 20px;">
			<select name="type_caisse" class="" id="type_caisse" style="width: 240px; ">
				<?php
				foreach($types_caisses as $ref=>$name):
					print "<option value=\"{$ref}\">{$name}</option>";
				endforeach;
				?>
			</select><span style="display: inline-block;width:10px;"></span>
			Dimensions : 
			<select name="reference" class="" id="reference" style="width: 240px; ">
				<?php
				foreach($references as $ref=>$name):
					print "<option value=\"{$ref}\">&nbsp;&nbsp;&nbsp; {$name}</option>";
				endforeach;
				?>
			</select><span style="display: inline-block;width:10px;"></span> Numéro de caisse : <input name="idno" maxlength="255" class="" style="width: 180px; height: 16px;"></input>
			...
			<div class="control-box-left-content">
				<a href="#"
				   ...
				   class="form-button 1506021943"><span class="form-button">
                        ...
                        <span class="form-button"><?php print $message; ?></span></a>
			</div>
		</div>
	</form>
	<?php
...
?>
    <div style="margin-top:70px;clear:both;">
        <h3>Ajouter à une caisse déjà existante <small><?php print sizeof($caisses);?> disponible<?php print (sizeof($caisses)>1 ? "s" : ""); ?></small></h3>
<?php
    foreach($caisses as $vt_caisse) {
        ...
        print "<p style='font-size: 0.9em;margin:0;padding:0;text-align: center;'>".str_replace(" mm","",$vt_caisse->get("ca_objects.dimensions.dimensions_depth")."x".$vt_caisse->get("ca_objects.dimensions.dimensions_width")."x".$vt_caisse->get("ca_objects.dimensions.dimensions_height"))."</p>";
